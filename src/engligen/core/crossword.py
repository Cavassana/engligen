...
def _can_place_dynamically(word: str, r_start: int, c_start: int, d_name: str, grid: Dict, directions: Dict, max_size: Tuple[int, int], bounds: Tuple) -> bool:
    ...
    # --- INÍCIO DA CORREÇÃO ---
    # O bloco de código abaixo foi removido. Ele era muito restritivo e impedia
    # a grade de crescer além da primeira palavra.
    '''
    intersections = 0
    for i in range(word_len):
        if grid.get((r_start + i * dr, c_start + i * dc)):
            intersections += 1
    
    if intersections == 1:
        support_points = 0
        for i in range(word_len):
            r_check, c_check = r_start + i * dr, c_start + i * dc
            if grid.get((r_check, c_check)):
                continue
            
            perp_dr, perp_dc = directions['vertical' if d_name == 'horizontal' else 'horizontal']
            if grid.get((r_check - perp_dr, c_check - perp_dc)) or grid.get((r_check + perp_dr, c_check + perp_dc)):
                support_points += 1
